...
require __DIR__ . '/TestObject.class.php';

$testObject0 = new TestObject();
...
// Getting all 12 test objects, sending into calculateTimeInState and breaking from the loop
// if there are no more objects
for ($count = 0; ; $count++) {
    $object = isset(${'testObject' . $count}) ? ${'testObject' . $count} : '';

    if (!$object) {
        print "\e[92mCalculating time for each state was a success\e[0m\n";
        break;
    }

    if (is_a($object, 'SomeObject')) {
        $result = StateUtils::calculateTimeInState($object, $count);
        $expected = ${'answer' . $count};
        $diff = $expected - $result;

        print 'Test ' . $count;
        print ($expected === $result ? " \e[92mPASSED\e[0m" : " \e[91mFAILED\e[0m");
        print "\tExpected: $expected | Result: $result | Difference: $diff";
        print "\n";
    } else {
        print "\e[91mPassed in object was not SomeObject\e[0m\n";
    }
}
